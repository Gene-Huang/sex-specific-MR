...
## add pleiotropy effect
## fixed variant-exposure effect
## male: variant-exposure effect size = 0.1
## set 50% of variants having sex-differences in variant-exposure effect 
## if a variant has sex-differences IV effect: female IV effect = male IV effect/2 = 0.05
## test different causal effect for female and male (female = 0, 0.05, 0.15; male = 0.1)
## competing methods:
...
## number of variants have pleiotropy effect
## 20 variants
number.pleiotropy <- 20
...
for (i in 1:RP){
  ...
  ####################################################
  ## pleiotropy effect
  ## randomly select 20 variants have pleiotropy effect
  ####################################################
  alpha <- rep(0, N.IV)
  
  alpha[sample(1:N.IV, number.pleiotropy, replace = F)] <- rnorm(number.pleiotropy, 0, 0.1)
  
  ## generate outcome
  Outcome.F <- 1 + alpha %*% t(P2.G.F) + True.Causal["Female"]*P2.Exposure.F + Sim.U.to.O*P2.U.F + rnorm(P2.Sample.F, 0, 1)
  
  Outcome.M <- 1 + alpha %*% t(P2.G.M) + True.Causal["Male"]*P2.Exposure.M + Sim.U.to.O*P2.U.M + rnorm(P2.Sample.M, 0, 1)
